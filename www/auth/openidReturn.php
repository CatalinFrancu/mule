...
if ($loggedInUser) {
  // user is logged in and trying to add a new OpenID to their account
  if (!$user->id) {
    // good, we haven't seen this OpenID  before
    $identity->userId = $loggedInUser->id;
    $identity->save();
    FlashMessage::add(_('New OpenID added to your account.'), 'info');
    ...
    FlashMessage::add(_("This OpenID is already linked to another account. We cannot merge accounts. " .
                        "Please close the other account before adding the OpenID to this one."));
  }
